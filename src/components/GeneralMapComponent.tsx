  ...
  return (
    <div className="w-full h-full relative">
      ...
      >
        ...
        {activePopupChurch && createPortal(
          <>
            <div
              className="fixed inset-0 bg-black/40 backdrop-blur-xs z-[9998]"
              onClick={() => setActivePopupChurch(null)}
            />
            <ChurchDetailModal
      ...
      {/* Floating Controls Container (Top Spacing Safe for Navigation Bar: top-20 sm:top-24, z-[1025]) */}
      <div className="absolute top-20 sm:top-24 right-3 md:right-4 z-[1025] flex items-center gap-2">
...
export default function GeneralMapComponent({
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-zinc-50 relative">
      ...
      <header className="absolute top-1 sm:top-2 left-1/2 -translate-x-1/2 w-[98%] sm:w-[95%] md:w-[90%] max-w-6xl mx-auto mt-1 sm:mt-2 md:mt-3 z-[1020] bg-white/85 dark:bg-slate-900/85 backdrop-blur-md border border-zinc-200 dark:border-slate-800 shadow-xl rounded-full p-1.5 sm:p-3 flex flex-row items-center justify-between gap-1.5 md:gap-3 transition-all duration-300">
        {/* Left Section: Logo & Counter */}
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          <div className="flex items-center space-x-1.5 sm:space-x-2">
            <img src="/img/logo.png" alt="IPDA" className="h-7 sm:h-10 w-auto object-contain" />
            <div>
              <h1 className="text-[10px] sm:text-xs font-black text-zinc-950 dark:text-white tracking-tight leading-tight whitespace-nowrap">
                GEOLOCALIZAÇÕES IPDA
              </h1>
              <p className="hidden sm:block text-[9px] text-zinc-500 dark:text-slate-400 font-semibold">GESTÃO DE DADOS</p>
            </div>
          </div>
          <button
            onClick={handleSyncDatabase}
            disabled={isSyncing}
            className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs font-semibold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-slate-800 border border-indigo-100 dark:border-slate-700 rounded-full hover:bg-indigo-100 dark:hover:bg-slate-700 transition-all active:scale-95 disabled:opacity-60 shadow-xs shrink-0"
            title="Clique para recarregar dados do banco em tempo real"
          >
            <RefreshCw className={`w-3 h-3 sm:w-3.5 sm:h-3.5 text-indigo-600 dark:text-indigo-400 ${isSyncing ? 'animate-spin' : ''}`} />
            <span>
              {isSyncing ? (
                'Sincronizando...'
              ) : (
                <>
                  <span className="hidden sm:inline">{filteredIgrejas.length} no mapa</span>
                  <span className="sm:hidden">{filteredIgrejas.length}</span>
                </>
              )}
            </span>
          </button>
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0 justify-end">
          <a
            href="/organizacao"
            className="flex items-center justify-center gap-1.5 bg-white/90 hover:bg-white text-slate-700 dark:text-slate-200 dark:bg-slate-800 font-medium text-xs px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl border border-slate-200/80 dark:border-slate-700 shadow-sm transition-all whitespace-nowrap min-h-[36px] sm:min-h-[44px]"
            title="Acessar Árvore da Organização"
          >
          ...
          <button
            ...
            className={`px-2.5 sm:px-3 py-1.5 rounded-xl border text-xs font-bold transition-all flex items-center justify-center gap-1.5 min-h-[36px] sm:min-h-[44px] ${
              showFilters
                ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm'
          ...
          {isAuthenticated && userRole === 'admin' ? (
            <a
              href="/validacao"
              className="p-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl border border-indigo-600 transition-all flex items-center justify-center shrink-0 gap-1.5 px-2.5 sm:px-3 shadow-xs hover:shadow-sm min-h-[36px] sm:min-h-[44px]"
            ...
            <button
              ...
              className="p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-200 rounded-xl border border-slate-200 dark:border-slate-700 transition-all flex items-center justify-center shrink-0 gap-1.5 px-2.5 sm:px-3 shadow-xs min-h-[36px] sm:min-h-[44px]"
