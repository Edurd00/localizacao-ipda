    ...
    <div className="relative w-full">
      ...
      <div className="relative">
        <input
          ...
          className="w-full text-xs font-semibold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg pl-7 pr-3 py-1.5 outline-none focus:ring-1 focus:ring-indigo-500 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500"
        />
        <Search className="absolute left-2 top-2 h-3.5 w-3.5 text-slate-400 dark:text-slate-500" />
      </div>

      {isOpen && filtered.length > 0 && (
        <>
          <div className="fixed inset-0 z-[1040]" onClick={() => setIsOpen(false)} />
          <div className="absolute top-full left-0 right-0 mt-1 max-h-40 overflow-y-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-lg z-[1050] divide-y divide-slate-100 dark:divide-slate-800">
            {filtered.map((ig) => (
              <button
                key={ig.codigo_totvs}
                type="button"
                ...
                className={`w-full text-left p-2 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-medium transition-colors ${
...
export default function RouteCompareModal({
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 top-auto md:absolute md:top-24 md:right-6 md:bottom-auto md:left-auto w-full md:w-96 rounded-t-3xl md:rounded-3xl border-t md:border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 shadow-2xl p-4 md:p-5 space-y-3 z-[1030] max-h-[85vh] overflow-y-auto duration-300 animate-in slide-in-from-bottom md:slide-in-from-top-2 flex flex-col">
        {/* Modal Header */}
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2.5 gap-4 shrink-0">
          ...
          <button
            type="button"
            onClick={handleCloseCompareModal}
            className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-2 min-h-[40px] min-w-[40px] flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-all"
        ...
        <div className="space-y-1 text-xs shrink-0">
          ...
          <span className="font-extrabold text-slate-900 dark:text-white block truncate" title={fixedDest.desc_igreja}>
            {fixedDest.desc_igreja}
          </span>
        </div>
        ...
        <div className="space-y-3 pt-1 border-t border-slate-100 dark:border-slate-800">
          {/* Sede Atual Card */}
          <div className="p-3 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-xl space-y-2 border-l-4 border-l-orange-500">
            <div className="flex items-center justify-between flex-wrap gap-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[9px] font-bold text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/60 px-2 py-0.5 rounded border border-orange-200 dark:border-orange-800">
                  Sede Atual (Vinculada)
                </span>
                {shortestOption === 'atual' && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-orange-800 dark:text-orange-300 bg-orange-50 dark:bg-orange-950/60 px-2 py-0.5 rounded-full border border-orange-200 dark:border-orange-800">
                    ⚡ Mais Próxima
                  </span>
                )}
              </div>
            ...
            <div className="flex items-center justify-between gap-2">
              <span className="text-[10px] text-slate-600 dark:text-slate-300 font-extrabold block truncate max-w-[150px]">
              ...
              {parentChurch && parentChurch.latitude && parentChurch.longitude && (
                <a
                  ...
                  className={`px-3 py-1.5 text-xs font-bold rounded-lg flex items-center justify-center gap-1 transition-all border ${
                    comparisonTransportMode === 'bus'
                      ? 'bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-200 hover:bg-amber-100 dark:hover:bg-amber-900/50'
                      : 'bg-indigo-600 dark:bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold shadow-md'
                  }`}
          ...
          <div className="p-3 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-xl space-y-2 border-l-4 border-l-emerald-500">
            <div className="flex items-center justify-between flex-wrap gap-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[9px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-200 dark:border-emerald-800">
                  Candidata A
                </span>
                {shortestOption === 'A' && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-emerald-800 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-800 animate-pulse">
            ...
            {sedeCandidataA && (
              <div className="space-y-2 pt-1 flex flex-col">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[10px] text-slate-900 dark:text-white font-extrabold block truncate max-w-[180px]" title={sedeCandidataA.desc_igreja}>
                ...
                {metaAtual && metaCandidataA && (
                  <div className="flex flex-col items-center justify-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                    {metaAtual.distance - metaCandidataA.distance > 0 ? (
                      <span className="text-[10px] font-extrabold text-emerald-800 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/60 px-3 py-1 rounded-full border border-emerald-200 dark:border-emerald-800 text-center w-full max-w-[260px]">
                        🟢 {(metaAtual.distance - metaCandidataA.distance).toFixed(1)}km mais perto
                      </span>
                    ) : (
                      <span className="text-[10px] font-extrabold text-rose-800 dark:text-rose-300 bg-rose-50 dark:bg-rose-950/60 px-3 py-1 rounded-full border border-rose-200 dark:border-rose-800 text-center w-full max-w-[260px]">
                    ...
                    {isAuthenticated ? (
                      <button
                        ...
                        className="bg-indigo-600 dark:bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold shadow-md w-full py-2 rounded-lg transition-all flex items-center justify-center gap-1.5 text-xs"
                        title="Gravar nova vinculação hierárquica"
                      >
                        <span>🔄 Transferir Coligação para Candidata A</span>
                      </button>
                    ) : (
                      <span className="text-[10px] text-slate-400 dark:text-slate-500 font-extrabold italic bg-slate-100 dark:bg-slate-800/85 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-xl text-center w-full">
          ...
          <div className="p-3 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-xl space-y-2 border-l-4 border-l-cyan-500">
            <div className="flex items-center justify-between flex-wrap gap-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[9px] font-bold text-cyan-600 dark:text-cyan-400 bg-cyan-50 dark:bg-cyan-950/60 px-2 py-0.5 rounded border border-cyan-200 dark:border-cyan-800">
                  Candidata B
                </span>
                {shortestOption === 'B' && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-cyan-800 dark:text-cyan-300 bg-cyan-50 dark:bg-cyan-950/60 px-2 py-0.5 rounded-full border border-cyan-200 dark:border-cyan-800 animate-pulse">
            ...
            {sedeCandidataB && (
              <div className="space-y-2 pt-1 flex flex-col">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[10px] text-slate-900 dark:text-white font-extrabold block truncate max-w-[180px]" title={sedeCandidataB.desc_igreja}>
        ...
        <div className="pt-2 border-t border-slate-100 dark:border-slate-800 text-[9px] text-slate-500 dark:text-slate-400 leading-normal shrink-0 font-medium">
